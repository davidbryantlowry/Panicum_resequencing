...
INDIR="/scratch/01499/oakglade/hallii_combined/09-gatk-first-unified-snps/split-samples/"
OUT="/scratch/01499/oakglade/hallii_combined/09-gatk-first-unified-snps/rawSNP-Q30.vcf"
...
for fil in ${INTERVALS}*.intervals; do
  BASE=$(basename $fil)
  NAME=${BASE%.*}
  echo "-V ${INDIR}rawINDEL-Q30-${NAME}.vcf \\" >> $JOB
done
